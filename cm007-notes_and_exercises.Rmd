...
```{r}
ggplot(gapminder, aes(x=year, y=lifeExp, colour=continent)) + geom_smooth(method="lm", se=FALSE) + geom_point()
```
...
```{r}
ggplot(gapminder, aes(x=year, y=lifeExp)) + facet_wrap(~ continent) + geom_point() + geom_smooth(se=FALSE, span=0.5)
```
...
```{r}
ggplot(gapminder, aes(x=year, y=pop, group=country, colour=gdpPercap)) + geom_line(alpha=0.25)
```
__Exercise 6__: Add points to the plot in Exercise 5.
```{r}
ggplot(gapminder, aes(x=year, y=pop, group=country, colour=gdpPercap)) + geom_line(alpha=0.25) + geom_point() + scale_y_log10()
```
